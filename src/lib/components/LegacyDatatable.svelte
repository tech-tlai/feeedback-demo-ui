<script>
	...
	export let customTheme = {
		// headerBg:tailwind-class
		// headerText:tailwind-class,
		// bodyText:tailwind-class
	};
	export let headerTheme = 'neutral'; // 'positive', 'negative', 'neutral'
	export let bodyTheme = ''; // 'custom'
	// WARNING: PROVIDE cutomTheme prop for  HEADER AND BODY THEME HAS TO WORK
	...
	export let highlightColor = 'rgb(249 250 251)';
	...
	$: headerStyle =
		{
			positive: 'bg-green-light text-gray-dark',
			negative: 'bg-red-light text-gray-dark',
			neutral: 'bg-gray-light text-gray-dark',
			custom:
				Object.keys(customTheme)?.length > 0
					? `!${customTheme.headerBg} !${customTheme.headerText}`
					: ''
		}[headerTheme] || 'bg-[#FFF5F5] text-gray-dark';

	$: bodyStyle =
		{
			custom: Object.keys(customTheme)?.length > 0 ? `!${customTheme.bodyText}` : ''
		}[bodyTheme] || 'text-gray-dark';
							...
							<tr>
								{#if tableHeadersDisplay}
									{#each tableHeadersDisplay as header (header.key)}
										<th
											scope="col"
											class=" table-header hover:bg-blue-300-hover"
										...
										>
											<button
												...
												class={`px-4 text-left text-sm font-medium  w-full flex items-center relative ${headerStyle} `}
											...
											>
												...
												{#if typeof sortAccordingTo.header === 'string' && sortAccordingTo.header === header.key}
													{#if sortAccordingTo.sortingOrder === 'descending'}
														<GoogleMatrialIcon iconName={'south'} addClass="text-sm" />
														<!-- {:else}
												Hello -->
													{/if}
													{#if sortAccordingTo.sortingOrder === 'ascending'}
														<GoogleMatrialIcon iconName={'north'} addClass="text-sm" />
														<!-- {:else}
												hello -->
													{/if}
												{:else}
													<GoogleMatrialIcon
														iconName={'sync_alt'}
														addClass="text-sm rotate-90 sort-icon "
													/>
													<!-- {/if} -->
												{/if}
											</button>
								...
								<th
									scope="col"
									class="px-4 text-left text-sm font-semibold text-gray-dark"
									class:py-2={rowHeight === 'compact'}
									class:py-3={rowHeight === 'normal'}
									class:py-4={rowHeight === 'medium'}
								>
						...
						<tbody class="divide-y divide-gray-200 bg-white leading-5">
							{#if currentPageData}
								{#each currentPageData as data, index}
									<tr
										style="background-color: {index % 2 === 0 ? highlightColor : 'transparent'};  "
									>
										{#each tableHeadersDisplay as header}
											<td
												class={`break-words pl-4 pr-3 text-sm  ${bodyStyle}`}
												class:py-2={rowHeight === 'compact'}
												class:py-3={rowHeight === 'normal'}
												class:py-4={rowHeight === 'medium'}
											>
											...
											<div class="flex gap-4 justify-center leading-3">
												{#each actionConfigObject as action}
													<button
														...
														class="text-accent-blue hover:text-indigo-900"
													>
														<GoogleMatrialIcon
		...
		<div class="flex items-center justify-between mt-4 px-4 py-3 sm:px-6">
			<div class="flex flex-1 justify-between sm:hidden">
				<button
					...
					class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-dark hover:bg-gray-50"
					>&lt;</button
				>
				<button
					on:click={() => handlePageIncrement(numberOfPages)}
					class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-dark hover:bg-gray-50"
					>&gt;</button
				>
			</div>
			<div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
				...
				<div>
					<nav class="isolate inline-flex -space-x-px rounded-md" aria-label="Pagination">
						<button
							on:click={() => handlePageDecrement()}
							class="relative inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 hover:bg-gray-50 focus:z-20 min-w-12 justify-center gap-2"
						>
							<span class="sr-only">Previous</span>

							<svg
								class="h-5 w-5 fill-gray-dark"
								viewBox="0 0 20 20"
								fill="currentColor"
								aria-hidden="true"
							>
							...
							<!-- <h1 class="text-sm flex items-center text-gray-dark">&lt;</h1> -->
						</button>
						<Select
                ...
                {/each} -->
						<button
							on:click={() => handlePageIncrement(numberOfPages)}
							class="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-dark hover:bg-gray-50 focus:z-20 min-w-12 justify-center gap-2"
						>
							<span class="sr-only">Previous</span>
							<!-- <h1 class="text-sm flex items-center text-gray-dark">&gt;</h1> -->
							<svg
