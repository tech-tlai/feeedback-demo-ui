...
<div class="rounded-lg w-full">
	<div class="relative flex gap-8">
		...
		<input
		type="text"
		class=" border border-gray-light rounded-md sm:rounded-lg shadow-sm px-12 py-2 w-full text-xs sm:text-base  focus:ring-0.5  focus:ring-primary  "
		{placeholder}
		bind:value={searchBoxValue}
		on:input={handleSearchValue}
	/>

	<SearchSvg strokeColor={'stroke-orange-100'} />
	...
	{#if searchButton}
	<button class="px-6 py-2 font-semibold text-white bg-primary rounded-[4px] sm:inline-block hidden">Search</button>
	{/if}
	</div>
</div>
